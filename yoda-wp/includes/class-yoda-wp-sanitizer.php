 ...
 *
 * @package    Yoda WP
 * @subpackage Yoda WP/includes
 */
class Yoda_WP_Sanitize {
    /**
    ...
    public function clean() {
         ...
         */
        do_action( 'yoda_wp_pre_sanitize', $sanitized );
        switch ( $this->type ) {
            case 'color'            :
            case 'radio'            :
         ...
         */
        do_action( 'yoda_wp_post_sanitize', $sanitized );
        return $sanitized;
    } // clean()
    /**
    ...
    public function set_type( $type ) {
        $check = '';
        if ( empty( $type ) ) {
            $check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'yoda-wp' ) );
        }
        if ( is_wp_error( $check ) ) {
            wp_die( $check->get_error_message(), __( 'Forgot data type', 'yoda-wp' ) );
        }
        $this->type = $type;
    } // set_type()
} // class
